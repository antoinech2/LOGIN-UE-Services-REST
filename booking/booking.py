...
def get_bookings_byid(userid):
   ...
   if user_bookings:
      ...
      return make_response(jsonify({"error":"Bookins not found"}), 400)
...
def add_booking(userid):
   ...
   service = requests.get(f"http://127.0.0.1:3202/showtimes/{req_date}")

   # check if there is an error using the response code
   if service.status_code != 200:
      return make_response(jsonify({"error":"Error in showtimes service"}), 400)
   
   service_request = service.json()
   movies = service_request["movies"]

   # check if the movie is in the list
   if movieid not in movies:
      res = make_response(jsonify({"error":"Movie not available for the selected date"}), 400)
   else :
